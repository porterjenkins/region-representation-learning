...
W, t, sources, ends = region_grid.create_flow_matrix(c['raw_flow_file'], region_name=c['city_name'], time=8,
                                                     sources=True)

size = region_grid.grid_size * region_grid.grid_size

time_sequences = random_walk_time(t, source_v=sources, walk_length=walk_length,
                                  n_walks=n_walks)
...
for r in region_grid.matrix_idx_map.values():
    ...
    if r not in sources.union(ends):
        sequences.append((f"{str(r)} " * walk_length).split(" "))

for seq in distance_sequences + time_sequences:
    s = [str(id) for id in seq]
    # print(list(s) + "\n")
    sequences.append(s)
